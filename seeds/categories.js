...
exports.seed = async (knex) => {
  ...
  try {
    ...
    await Category.query().upsertGraph(categoryParents, {
      noDelete: true,
      relate: true,
      allowRefs: true,
    });
  ...
  if (!SEED_SUFFIX) {
    ...
    getEntitiesFromCsv(recommendationsCsv, { delimiter: DELIMITER })
      .filter((entity) => entity.minValue || entity.maxValue)
      .forEach(async (entity) => {
        const attributeId = attributes.find(
          (attribute) =>
            attribute.name['en-US'].toLocaleLowerCase() === entity['attribute.name["en-US"]'].toLocaleLowerCase()
        )?.id;
